...
import { checkContractFreshness } from "../contract/check-freshness.js";
...
export async function runAgentDriftChecks(
  projectRoot = getProjectRoot(),
): Promise<CheckViolation[]> {
  const staleFiles = await checkContractFreshness(projectRoot);

  const freshnessViolations = staleFiles.map<CheckViolation>((file) => ({
    check: "contract-stale",
    file,
    message:
      "Generated contract artifact is stale. Run `npm run contract:generate`.",
  }));
...
async function main(): Promise<void> {
  ...
  const violations = await runAgentDriftChecks(projectRoot);
