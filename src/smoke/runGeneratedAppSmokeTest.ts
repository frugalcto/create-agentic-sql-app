...
import type {
  GeneratedAppSmokeTestOptions,
  GeneratedAppSmokeTestResult,
  SmokeTestContext,
} from "./types.js";
...
function createComposeProjectName(): string {
  return `agentic-smoke-${randomUUID().slice(0, 8)}`;
}

function createContainerName(composeProjectName: string): string {
  return `${composeProjectName}_postgres`;
}

function composeEnvironment(composeProjectName: string): NodeJS.ProcessEnv {
  return {
    ...process.env,
...
export async function cleanupSmokeTest(
  ...
  if (context.appDir && context.composeProjectName) {
    try {
      await execa(
        "docker",
        ["compose", "-p", context.composeProjectName, "down", "-v"],
        {
          cwd: context.appDir,
          stdio: "pipe",
...
export async function runGeneratedAppSmokeTest(
  options: GeneratedAppSmokeTestOptions = {},
): Promise<GeneratedAppSmokeTestResult> {
  await checkDockerAvailable();

  const context: SmokeTestContext = {};
  const steps: string[] = [];
  ...
  try {
    ...
    await runStep("Prepare environment file", steps, async () => {
      ...
      await patchDockerComposeContainerName(
        appDir,
        createContainerName(composeProjectName),
      );
    });
